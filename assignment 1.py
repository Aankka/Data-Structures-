      #Asafo-Adjei Nana Kwabena Kwarteng
...
#Computes for Shear Force and Bending Moment when x = 0
...
#Computes for Shear Force and Bending Moment when x = 12
...
x = V + (L/2)  # Manipulation of original V equation to make x the subject
...
w = 10
beam_span = np.arange (0, beam_length + steps, steps) # list to discretize the beam length
x = beam_span #The beam_span is every point on the beam starting from 0 and ending at 12m
M = w * (-6*x**2 + 6*L*x - L**2)/12 #Equation for moment
print()
...
max_BM = max(Moment_values) #Find the maximum bending moment in the list of moment values
min_BM = min(Moment_values) #Find the minimum bending moment in the list of moment values
